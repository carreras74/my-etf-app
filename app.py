...
# 3. 매입장부 데이터 로드 (만능 차트 함수에서 공통 사용)
...
# 💡 [핵심 패치] 입체 분석 차트를 그리는 '만능 함수' 제작
def render_stock_3d_chart(stock_name, etf_data, ledger_df):
    ...
    fig.add_trace(go.Scatter(x=valid_p_df['Date'], y=valid_p_df['Price'], name='주가(원)', mode='lines+markers', line=dict(color='#333333', width=3), marker=dict(size=6)), row=1, col=1, secondary_y=True)
    ...
    if not p_df.empty:
        if buy_price is not None:
            fig.add_trace(go.Scatter(x=[p_df['Date'].iloc[0], p_df['Date'].iloc[-1]], y=[buy_price, buy_price], mode="lines+text", line=dict(color="#00C853", dash="dash", width=2), name=f"내 평단가 ({buy_price:,.0f}원)", text=[f"내 매수단가 ({buy_price:,.0f}원)", ""], textposition="bottom right", showlegend=False, hoverinfo="skip"), row=1, col=1, secondary_y=True)
        if buy_date is not None and buy_date in p_df['Date'].values:
            ...
            fig.add_trace(go.Scatter(x=[buy_date, buy_date], y=[min_y - margin, max_y + margin], mode="lines+text", line=dict(color="#00C853", dash="dash", width=2), name="매수일자", text=["매수타점", ""], textposition="top right", showlegend=False, hoverinfo="skip"), row=1, col=1, secondary_y=True)

    fig.update_layout(height=650, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), plot_bgcolor='white', paper_bgcolor='white', margin=dict(l=10, r=10, t=50, b=10))
    ...
    fig.update_yaxes(title_text="총 수량증감", row=2, col=1, showgrid=True, gridcolor='#F0F0F0', zeroline=True, zerolinecolor='#E0E0E0')
...
# =====================================================================
# 🔥 [섹션 2: 수급 격전지 찐 주도주 분석 (매수금액 기준 업데이트)]
# =====================================================================
st.markdown("---")
st.header("🔥 최근 5영업일 누적 순매수 찐 주도주 TOP 20 (단위: 백만원)")
st.markdown("**💡 비중(%p)이 아닌 실제 투입된 '매수 금액(주식수증가 × 당일종가)'을 계산하여 진짜 큰돈이 몰린 종목을 추적합니다.**")
...
def draw_top20_bar_chart(records, category_name, color_map):
    ...
    df_res = df_res.sort_values(by='5일매수(백만)', ascending=False).head(20)
    ...
    fig = px.bar(
        ...
        title=f"{title_emoji} [{category_name}] 5일 누적 순매수 찐 주도주 TOP 20 ({date_str} 기준)",
        color_discrete_map=color_map
    )
    
    fig.update_layout(xaxis_title="", yaxis_title="순매수 금액 (백만원)", height=650, legend_title="매집 기간")
    ...
    with st.expander("🦅 [히든 대시보드] TIME TOP 20 찐 주도주 입체 분석 차트 열어보기"):
        ...
        if time_top20_stocks:
            ...
            for i, tab in enumerate(tabs):
                with tab:
                    render_stock_3d_chart(time_top20_stocks[i], etf_data, ledger_df)
...
# 💡 [핵심 패치] KoAct TOP 20 입체 분석 히든 파일
if not top20_koact_df.empty:
    with st.expander("🦅 [히든 대시보드] KoAct TOP 20 찐 주도주 입체 분석 차트 열어보기"):
        ...
        if koact_top20_stocks:
            ...
            for i, tab in enumerate(tabs):
                with tab:
                    render_stock_3d_chart(koact_top20_stocks[i], etf_data, ledger_df)


# =====================================================================
# 🕵️‍♂️ [섹션 2-1: TOP 20 종목 한 달(20영업일) 상세 매매/가격 내역]
# =====================================================================
...
if not top20_combined.empty:
    with st.expander("🔍 [히든 데이터베이스] 위 TOP 20 찐 주도주들의 한 달(최근 20일) 상세 거래 장부 열어보기"):
        st.markdown("**💡 선택된 핵심 종목들이 지난 한 달간 매일 수량과 가격이 어떻게 변했는지 상세하게 추적합니다. (최신 날짜순)**")
        ...
        if history_list:
            ...
            hist_df = hist_df.sort_values(by=['일자', '순매수액(백만원)'], ascending=[False, False])
            st.dataframe(hist_df, use_container_width=True, hide_index=True)
        else:
            st.info("해당 종목들의 최근 한 달간 유의미한 매매 변동 내역이 없습니다.")
    ...
    if not ledger_df.empty:
        ...
        if my_stocks and etf_data:
            ...
            for i, tab in enumerate(stock_tabs):
                with tab:
                    # 💡 [핵심 패치] 여기서도 만능 함수를 그대로 호출하여 깔끔하게 처리!
                    render_stock_3d_chart(my_stocks[i], etf_data, ledger_df)
        else:
