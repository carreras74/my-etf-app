...
# 💡 [수정됨] 관심종목 시트가 15개로 잘리는 것을 막기 위해 원본 전체를 미리 빼둡니다!
...
# 구글 시트에서 데이터를 읽어올 때 필터링 (관심종목도 15일 자르기 로직에서 제외)
filtered_etf_data = {}
...
for etf_name, df in etf_data.items():
    if len(df.columns) > 0:
        ...
        # 여기서 일반 ETF 장부들은 최신 15일치만 남기게 됩니다.
        df = df.tail(15)
...
# 1. 💡 위에서 15개로 잘리지 않고 안전하게 빼둔 원본 전체 데이터를 가져옵니다!
