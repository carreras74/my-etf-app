...
# 스트림릿 바탕화면을 강제로 고급스러운 다크톤으로 맞추는 CSS 패치
st.markdown("""
<style>
...
# =====================================================================
# 💡 [핵심 패치] 3단(비중/수량/금액) 입체 분석 차트 만능 함수 (다크모드)
# =====================================================================
def render_stock_3d_chart(stock_name, etf_data, ledger_df):
    ...
    for etf_name, df in etf_data.items():
        if stock_name in df.columns:
            ...
            if m_weight > max_weight: max_weight = m_weight; best_etf = etf_name
    ...
    for etf_name, df in etf_data.items():
        ...
        for _, row in df.iterrows():
            ...
            # 💡 [업데이트] 수량 * 가격을 계산하여 순매수/매도 금액(단위: 백만원) 누적
            agg_data[d]['TotalAmtChange'] += (qty_change * price) / 1000000.0

    plot_data = [{'Date': k, 'Weight': v['Weight'], 'Price': v['Price'], 'QtyChange': v['TotalQtyChange'], 'AmtChange': v['TotalAmtChange']} for k, v in agg_data.items()]
    ...
    # 매수(빨강), 매도(파랑) 색상 세팅 (다크모드에 맞게 네온 느낌으로 조절)
...
def draw_top20_bar_chart(records, category_name, color_map):
    ...
    fig = px.bar(
        ...
        title=f"🔥 [{category_name}] 5일 누적 순매수 찐 주도주 TOP 20 ({date_str} 기준)",
    ...
    with st.expander("🦅 [히든 대시보드] TIME TOP 20 찐 주도주 입체 분석 차트 열어보기"):
        ...
        if time_top20_stocks:
            ...
            for i, tab in enumerate(tabs):
                with tab: render_stock_3d_chart(time_top20_stocks[i], etf_data, ledger_df)
...
if not top20_koact_df.empty:
    with st.expander("🦅 [히든 대시보드] KoAct TOP 20 찐 주도주 입체 분석 차트 열어보기"):
        ...
        if koact_top20_stocks:
            ...
            for i, tab in enumerate(tabs):
                with tab: render_stock_3d_chart(koact_top20_stocks[i], etf_data, ledger_df)
    ...
    if not ledger_df.empty:
        ...
        if my_stocks and etf_data:
            ...
            for i, tab in enumerate(stock_tabs):
                with tab: render_stock_3d_chart(my_stocks[i], etf_data, ledger_df)
    else:
