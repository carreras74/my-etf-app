...
# 💡 [핵심 패치 1] 수량백업 시트는 그래프용 데이터가 아니므로 따로 빼서 안전하게 보관합니다.
...
for etf_name, df in etf_data.items():
    if "수량백업" in etf_name: continue # 백업 시트 건너뛰기
    if len(df.columns) > 0:
...
def draw_momentum_bump_chart(target_cat, etf_dict, backup_df):
    ...
    # 💡 [업그레이드 2] 수량백업 시트에서 해당 운용사(브랜드)의 '현재 보유량' 총합을 계산합니다.
    brand_current_qty = {}
    ...
    # 💡 [업그레이드 3] 툴팁에 보여줄 '현재 보유량(주)'과 '보유총액(백만)'을 계산해서 붙여넣습니다.
    long_df['현재 보유량(주)'] = long_df['Stock'].apply(lambda s: f"{brand_current_qty.get(s, 0):,}")
    long_df['보유총액(백만)'] = long_df['Stock'].apply(lambda s: f"{(brand_current_qty.get(s, 0) * latest_prices.get(s, 0)) / 1000000.0:,.1f}")
    ...
    fig = px.line(
        ...
        # 💡 [업그레이드 4] 마우스를 올리면 새로 추가한 보유량과 보유총액까지 완벽하게 보여줍니다.
        hover_data={
            'Rank': True,
            '순매수(백만)': ':,.1f',
            '수량(5일 누적)': True, 
...
# 💡 TIME (주황) 렌더링 -> 바로 아래에 범프 차트 출력 (백업 DB도 같이 넘겨줍니다)
