...
# 1. etf_data 딕셔너리에서 '관심종목' 시트 데이터 가져오기
pullback_df = etf_data.get("관심종목", pd.DataFrame())

if not pullback_df.empty:
    st.success("✅ 눌림목 알고리즘이 포착한 종목이 업데이트되어 있습니다.")
    
    # 데이터프레임 시각화 (인덱스 숨김, 컨테이너 너비 맞춤)
    st.dataframe(pullback_df, use_container_width=True, hide_index=True)
    
    # 💡 [Pro Tip] 포착된 종목을 기존의 '2단 입체 차트'와 다이렉트 연동!
    if '종목명' in pullback_df.columns:
        # 포착된 종목 중 현재 ETF 유니버스(all_stocks_list)에 존재하는 종목만 필터링
        pb_stocks = [s for s in pullback_df['종목명'].tolist() if s in all_stocks_list]
        
        if pb_stocks:
            with st.expander("🦅 [히든 대시보드] 포착된 눌림목 종목 즉시 입체 분석 열기", expanded=False):
                tabs = st.tabs([f"📈 {s}" for s in pb_stocks])
                for i, tab in enumerate(tabs):
                    with tab:
                        # 기존에 만들어둔 3D 차트 렌더링 함수 재활용
