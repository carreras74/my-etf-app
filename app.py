...
# =====================================================================
# 3. 📊 [진짜 엔진] 5영업일 누적 "순매수/순매도 대금" 격전지 TOP 20 
# =====================================================================
st.markdown("---")
st.header("🔥 최근 5영업일 누적 순매수/순매도 격전지 TOP 20")
st.markdown("**💡 비중 착시 완벽 제거! 5일간 기관이 가장 격렬하게 돈을 움직인(크게 샀거나 크게 판) 진짜 주도주를 찾습니다.**")
...
def make_records(agg_dict):
    recs = []
    for stock, flows in agg_dict.items():
        # 💡 [핵심 패치] 매수(플러스)든 매도(마이너스)든 돈이 조금이라도 움직였으면 통과!
        if flows['etf_5d']: 
            # 💡 대장 ETF도 '절대값' 기준으로 가장 격렬하게 사고판 ETF를 찾습니다.
...
def draw_top20_money_chart(records, category_name, color_map):
    ...
    # 💡 [핵심 패치 2] 순위 정렬도 철저하게 '절대값(돈의 크기)' 기준으로 20개를 뽑습니다.
    ...
    fig = px.bar(
        ...
        title=f"{title_emoji} [{category_name} 그룹 전체통합] 5일 집중 매수/매도 격전지 TOP 20 ({date_str} 기준)",
        color_discrete_map=color_map
    )
    
    fig.update_layout(xaxis_title="", yaxis_title="누적 순매수 대금 (단위: 백만원)", height=650, legend_title="기간")
    # 마이너스(매도) 막대가 잘 보이도록 0원 기준선을 진하게 표시합니다.
...
if time_top20_stocks:
    with st.expander("🔍 [TIME 그룹] 격전지 TOP 20 종목별 다크모드 입체 분석 열기 (클릭)"):
...
if koact_top20_stocks:
    with st.expander("🔍 [KoAct 그룹] 격전지 TOP 20 종목별 다크모드 입체 분석 열기 (클릭)"):
...
try:
    ...
    st.info(f"💡 매입장부(Excel)를 불러올 수 없습니다. GitHub 저장소에 '매입장부.xlsx'가 있는지 확인하세요.")
