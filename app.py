    ...
    for col in change_cols:
        ...
        for _, row in recent_df.iterrows():
            ...
            if isinstance(val_str, str) and " | " in val_str:
                ...
                if '🔴▲' in q_str:
                    qty = int(q_str.replace('🔴▲', '').replace(',', '').strip())
                elif '🔵▼' in q_str:
                    qty = -int(q_str.replace('🔵▼', '').replace(',', '').strip())
        ...
        if amt_5d > 0 or amt_1d > 0: 
            record = {
                ...
                '종목명': stock_name,
                'ETF명': etf_name,
                '당일매수(백만)': round(amt_1d, 1),
                '3일매수(백만)': round(amt_3d, 1),
                '5일매수(백만)': round(amt_5d, 1)
            }
            if category == "TIME":
...
if not top20_combined.empty:
    with st.expander("🔍 [히든 파일] 위 TOP 20 찐 주도주들의 한 달(최근 20일) 상세 매매 및 가격 변동 내역 열어보기"):
            ...
            if e_name in etf_data:
                ...
                if col_name in df_raw.columns:
                    for _, r in df_raw.iterrows():
                        ...
                        if isinstance(val_str, str) and " | " in val_str:
                            parts = val_str.split(" | ")
                            q_str_disp = parts[0].strip()
                            p_str_disp = parts[1].strip()
                            
                            if '🔴▲' in q_str_disp:
                                qty = int(q_str_disp.replace('🔴▲', '').replace(',', '').strip())
                            elif '🔵▼' in q_str_disp:
                                qty = -int(q_str_disp.replace('🔵▼', '').replace(',', '').strip())
                        ...
                        # 변동이 있었던 날만 기록 (깔끔한 표를 위해)
                        if qty != 0:
                            history_list.append({
                                '일자': date_str,
                                '운용사(ETF)': e_name.replace('TIME ', 'TIME').replace('KoAct ', 'KoAct'),
                                '종목명': s_name,
                                '매수/매도 수량': q_str_disp,
                                '당일 종가': p_str_disp,
                                '순매수액(백만원)': round(daily_amt, 1)
                            })
                            
        if history_list:
            hist_df = pd.DataFrame(history_list)
            # 날짜 최신순, 순매수액 높은 순으로 정렬
