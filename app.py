...
def load_data_from_google():
    try:
        ...
        for ws in worksheets:
            ...
            # 💡 [핵심] 제목 줄에 중복이 있어도 에러 없이 자동으로 이름표를 붙여줍니다!
            headers = data[0]
            seen = {}
            new_headers = []
            for h in headers:
...
if etf_data:
    ...
    with st.expander("📊 구글 시트 원본 데이터 펴보기 (종목별 수량증감 상세 조회)"):
        st.markdown("**💡 구글 시트와 동일한 원본 데이터입니다.**")
        st.dataframe(raw_df, use_container_width=True, hide_index=True)


    # =====================================================================
    # 4. 🔥 최근 5영업일 누적 순매수대금 찐 주도주 TOP 20 
    # =====================================================================
    st.markdown("---")
    ...
    for etf_name, r_df in etf_data.items():
        if "TIME" in etf_name or "타임" in etf_name: cat_agg = time_agg
        elif "KoAct" in etf_name or "코액트" in etf_name: cat_agg = koact_agg
        else: continue
            
        df2 = r_df.copy()
        if len(df2.columns) <= 3: continue 
            
        d_col = df2.columns[0]
        df2[d_col] = pd.to_datetime(df2[d_col])
        df2 = df2.sort_values(by=d_col)
        
        if len(df2) == 0: continue
        all_dates.extend(df2[d_col].dt.strftime('%Y-%m-%d').tolist())
        
        last_5, last_3, last_1 = df2.tail(5), df2.tail(3), df2.tail(1)
        s_cols = [c for c in df2.columns if c != d_col and not str(c).endswith('_증감')]

        for stock in s_cols:
            diff_c = f"{stock}_증감"
            if diff_c not in df2.columns: continue
            if stock not in cat_agg:
                cat_agg[stock] = {'1d': 0.0, '3d': 0.0, '5d': 0.0, 'etf_5d': {}}
                
            def calc_net_buy(sub_df):
                n_buy = 0.0
                for val in sub_df[diff_c]:
                    if " | " in str(val):
                        parts = str(val).split(" | ")
            ...
            v_1d = calc_net_buy(last_1)
            v_3d = calc_net_buy(last_3)
            v_5d = calc_net_buy(last_5)
            
            cat_agg[stock]['1d'] += v_1d
            cat_agg[stock]['3d'] += v_3d
            cat_agg[stock]['5d'] += v_5d
            
            if etf_name not in cat_agg[stock]['etf_5d']: cat_agg[stock]['etf_5d'][etf_name] = 0.0
            cat_agg[stock]['etf_5d'][etf_name] += v_5d
    ...
    def make_records(agg_dict):
        ...
        for s, f in agg_dict.items():
            if f['5d'] > 0: 
                best_etf = max(f['etf_5d'], key=f['etf_5d'].get)
                etf_short = best_etf.replace('TIME ', '').replace('TIME', '').replace('KoAct ', '').replace('KoAct', '').strip()
                recs.append({
                    '기준일자': global_latest_date, '종목명': s, '표시명': f"{s}<br>({etf_short})",
                    '당일순매수(백만)': round(f['1d'] / 1000000, 1),
                    '3영업일순매수(백만)': round(f['3d'] / 1000000, 1),
                    '5영업일순매수(백만)': round(f['5d'] / 1000000, 1)
                })
        return recs

    time_records = make_records(time_agg)
    koact_records = make_records(koact_agg)

    def draw_top20_money_chart(records, category_name, color_map):
        if not records: return []
        df_res = pd.DataFrame(records).sort_values(by='5영업일순매수(백만)', ascending=False).head(20)
        date_str = df_res['기준일자'].iloc[0] if not df_res.empty else global_latest_date
        
        df_melted = df_res.melt(id_vars=['표시명'], value_vars=['5영업일순매수(백만)', '3영업일순매수(백만)', '당일순매수(백만)'], var_name='기간', value_name='순매수대금(백만원)')
        ...
        fig.update_layout(xaxis_title="", yaxis_title="누적 순매수 대금 (단위: 백만원)", height=650, legend_title="매집 기간", template="plotly_dark")
    ...
    time_top20 = draw_top20_money_chart(time_records, "TIME", color_time)
    if time_top20:
        with st.expander("🔍 [TIME 그룹] 주도주 TOP 20 종목별 다크모드 입체 분석 열기 (클릭)"):
            time_tabs = st.tabs([f"📈 {name}" for name in time_top20])
            for idx, tab in enumerate(time_tabs):
                with tab: draw_hts_chart(time_top20[idx], etf_data, unique_key=f"time_{time_top20[idx]}_{idx}")

    st.markdown("<br><br>", unsafe_allow_html=True)

    color_koact = {'5영업일순매수(백만)': '#AEC7E8', '3영업일순매수(백만)': '#1F77B4', '당일순매수(백만)': '#17BECF'}
    koact_top20 = draw_top20_money_chart(koact_records, "KoAct", color_koact)
    if koact_top20:
        with st.expander("🔍 [KoAct 그룹] 주도주 TOP 20 종목별 다크모드 입체 분석 열기 (클릭)"):
            koact_tabs = st.tabs([f"📈 {name}" for name in koact_top20])
            for idx, tab in enumerate(koact_tabs):
                with tab: draw_hts_chart(koact_top20[idx], etf_data, unique_key=f"koact_{koact_top20[idx]}_{idx}")


    # =====================================================================
    # 5. 🦅 내 매입 종목 입체 분석 대시보드 (매입장부 연동)
    # =====================================================================
    st.markdown("---")
    ...
    try:
        ...
        st.warning(f"⚠️ 매입장부 데이터를 불러오는 중 에러가 발생했습니다: {e}. 깃허브에 '매입장부.xlsx' 파일이 정상적으로 있는지 확인해 주세요.")

    st.markdown("<br><br><br>", unsafe_allow_html=True)
else:
    st.error("데이터를 불러오지 못했습니다. 구글 시트 연결 상태를 확인해 주세요.")
