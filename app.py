...
# =====================================================================
st.markdown("---")
st.header("🔥 최근 5영업일 누적 찐 주도주 & 역(逆) 주도주 TOP 20 (단위: 백만원)")
...
def draw_merged_bottom20(category_records, category_name, color_map):
    ...
    # 💡 순매도는 마이너스 금액이므로 '오름차순'으로 정렬해서 가져옵니다.
    res_df = grouped.sort_values(by='5일매수(백만)', ascending=True).head(20)
    res_df = res_df[res_df['5일매수(백만)'] < 0] # 매도(마이너스)인 것만 필터링
    ...
    fig.update_layout(font=dict(color="#FFFFFF"), template="plotly_dark", plot_bgcolor='#121212', paper_bgcolor='#121212', xaxis_title="", yaxis_title="합산 순매도 금액 (백만원)", height=650)
    fig.update_traces(textposition='outside', textangle=-90, textfont_size=10, texttemplate='%{text:,.1f}')
...
b20_time = draw_merged_bottom20(merged_data["TIME"], "TIME", {'5일매수(백만)': '#D1C4E9', '3일매수(백만)': '#9575CD', '당일매수(백만)': '#512DA8'}) # 순매도 컬러맵 (보라색 계열)

time_stocks = []
if not t20_time.empty: time_stocks.extend(t20_time['종목명'].tolist())
if not b20_time.empty: time_stocks.extend(b20_time['종목명'].tolist())
time_stocks = list(dict.fromkeys(time_stocks)) # 중복 제거

if time_stocks:
    with st.expander("🦅 [히든 대시보드] TIME 주도주(순매수+순매도) 정밀 입체 분석"):
...
b20_koact = draw_merged_bottom20(merged_data["KoAct"], "KoAct", {'5일매수(백만)': '#D1C4E9', '3일매수(백만)': '#9575CD', '당일매수(백만)': '#512DA8'}) # 순매도 컬러맵 (보라색 계열)
...
koact_stocks = list(dict.fromkeys(koact_stocks)) # 중복 제거

if koact_stocks:
    with st.expander("🦅 [히든 대시보드] KoAct 주도주(순매수+순매도) 정밀 입체 분석"):
...
# =====================================================================
st.markdown("---")
with st.expander("🔍 [히든 데이터베이스] 순매수/순매도 TOP 20 종목들의 개별 ETF 거래 장부 (어느 ETF가 사고 팔았나?)"):
    all_df_list = [df for df in [t20_time, b20_time, t20_koact, b20_koact] if not df.empty]
    if all_df_list:
        top20_all = pd.concat(all_df_list)['종목명'].unique()
        h_list = []
