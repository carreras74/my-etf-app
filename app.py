...
def render_stock_3d_chart(stock_name, etf_data, ledger_df, unique_key):
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color=colors, width=0.35, offsetgroup=1,
            customdata=p_df['QtyChange'],
            text=text_amt, textposition='outside', textfont=dict(color='white', size=10),
            hovertemplate='%{x}<br>순매수: %{y:,.0f} 백만원<br>수량: %{customdata:,.0f} 주<extra></extra>'
        ), 
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color=colors, width=0.35, offsetgroup=2, opacity=0.7,
            hoverinfo='skip'
        ), 
...
with st.expander("🦅 [히든 대시보드] 내가 원하는 종목을 골라서 분석 차트 열어보기", expanded=True):
    ...
    with col2:
        ...
        st.markdown("<div style='font-size: 13px; color: #AAAAAA; margin-bottom: 5px;'>💡 처음이신가요? 아래 양식을 다운받아 작성 후 올려보세요. (필수: 종목명, 매수일자, 매수단가)</div>", unsafe_allow_html=True)
        
        template_df = pd.DataFrame([['삼성전자', '2024-03-26', '75000']], columns=['종목명', '매수일자', '매수단가'])
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            template_df.to_excel(writer, index=False, sheet_name='매입장부')
        template_bytes = output.getvalue()
        
        st.download_button(
            label="📥 엑셀 양식 다운로드",
            data=template_bytes,
            file_name="매입장부_양식.xlsx",
