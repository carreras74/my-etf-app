...
# 💡 구글 시트에서 데이터를 읽어올 때 TIGER 무시 필터링
filtered_etf_data = {}
...
def draw_momentum_bump_chart(target_cat, etf_dict, backup_df):
    ...
    # 💡 [핵심 엔진 교체] 5일 합산 로직을 완전히 제거하고 '당일 데이터'만 사용하도록 변경!
    daily_pivot_amt = daily_sum.pivot(index='Date', columns='Stock', values='Amt').fillna(0).sort_index()
    daily_pivot_qty = daily_sum.pivot(index='Date', columns='Stock', values='Qty').fillna(0).sort_index()
    
    # 💡 당일 기준으로 매일 Top 10에 한 번이라도 이름을 올렸던 종목들 추출
    target_universe = set()
    for date in daily_pivot_amt.index:
        daily_top10 = daily_pivot_amt.loc[date].nlargest(10).index.tolist()
        target_universe.update(daily_top10)
        
    if not target_universe: return
    
    # 차트용 데이터 Melt
    universe_amt = daily_pivot_amt[list(target_universe)].reset_index().melt(id_vars='Date', var_name='Stock', value_name='Daily_Amount')
    universe_qty = daily_pivot_qty[list(target_universe)].reset_index().melt(id_vars='Date', var_name='Stock', value_name='Daily_Qty')
    long_df = pd.merge(universe_amt, universe_qty, on=['Date', 'Stock'])
    
    # 💡 [순위 변경] 이제 선의 움직임(순위)도 '당일 순매수 금액'을 기준으로 매일 요동칩니다!
    long_df['Rank'] = long_df.groupby('Date')['Daily_Amount'].rank(method='first', ascending=False)
    ...
    long_df['당일 수량(주)'] = long_df['Daily_Qty'].apply(format_qty_str)
    long_df = long_df.rename(columns={'Daily_Amount': '당일 순매수(백만)'})
    ...
    def calc_hist_val(r):
        ...
        p = historical_price.get((r['Date'], r['Stock']), latest_prices.get(r['Stock'], 0)) 
    ...
    fig = px.line(
        ...
        # 💡 타이틀도 '당일 기준'으로 명확하게 수정
        title=f"🏆 [{target_cat}] 찐 주도주 15일 수급 로테이션 (<b>당일 하루치</b> 순매수액 <b>Top 10</b> 기준)"
    )
    
    fig.update_yaxes(autorange="reversed", title="순위 (당일 금액 기준)")
