...
# 💡 [핵심 패치 1] 메인 차트의 세로 길이 자동 조절 로직 추가
num_stocks_main = len(latest_order)
dynamic_height_main = max(800, num_stocks_main * 25 + 100) # 종목당 25픽셀씩 늘려서 범례가 잘리지 않게 만듭니다!
...
fig.update_layout(
    ...
    height=dynamic_height_main, # 동적 높이 적용
    ...
    # 💡 yanchor를 "top"으로 변경하여 위에서부터 차곡차곡 정렬되게 세팅
...
def draw_momentum_bump_chart(target_cat, etf_dict, backup_df):
    ...
    # 💡 [핵심 패치 2] 주도주 모멘텀 차트 세로 길이 자동 조절 로직 추가
    ...
    fig.update_layout(
        font=dict(color="#FFFFFF"), template="plotly_dark", plot_bgcolor='#121212', paper_bgcolor='#121212',
        height=dynamic_height_mom, # 동적 높이 적용
        hovermode="closest", 
        # 💡 yanchor를 "top"으로 변경
